...
function DefaultIcon({ type, color }: { type: ToastType, color?: string }) {
  switch (type) {
    case 'success':
...
function useSpringTransition(visible: boolean) {
...
export function Toast({ toast, onDismiss }: ToastProps) {
  const { opacity, translateY, scale } = useSpringTransition(toast.visible);
  ...
  return (
    ...
    >
      {/* Badge */}
      <View style={[styles.badge, toast.styles?.badge]}>
        {toast.icon ?? <DefaultIcon type={toast.type} color={toast.colorIcon} />}
      </View>
      ...
      {toast.button ? (
        ...
        >
          <Text style={[styles.actionButtonText, toast.styles?.buttonText]}>
            {toast.button.title}
          </Text>
        </TouchableOpacity>
      ) : null}
